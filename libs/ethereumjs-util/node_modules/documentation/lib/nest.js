...
var tagDepth = function tagDepth(tag) {
 ...
 */
var nestTag = function nestTag(tags, errors
// Use lodash here both for brevity and also because, unlike JavaScript's
// sort, it's stable.
) {
  return _.sortBy(removeUnnamedTags(tags), tagDepth).reduce(function (memo, tag) {
    function insertTag(node, parts) {
      ...
      if (parts.length === 1) {
        ...
        // this tag.
        var child = node.properties && node.properties.find(function (property) {
          return parts[0] === _.last(property.name.split(PATH_SPLIT));
        });

        if (!child) {
          if (tag.name.match(/^(\$\d+)/)) {
            errors.push({
              message: `Parent of ${tag.name} not found. To document a destructuring\n` + `type, add a @param tag in its position to specify the name of the\n` + `destructured parameter`,
 ...
 * @returns {Object} nested comment
 */
var nest = function nest(comment) {
