...
var _parse_to_ast = require('./parse_to_ast');

var _ = require('lodash');
var t = require('babel-types');
var parse = require('../parse');
var walkComments = require('../extractors/comments');
var walkExported = require('../extractors/exported');
var util = require('util');
var debuglog = util.debuglog('documentation');
...
function parseJavaScript(data, config) {
  var visited = new Set();
  var commentsByNode = new Map();

  var ast = (0, _parse_to_ast.parseToAst)(data.source);
  var addComment = _addComment.bind(null, visited, commentsByNode);

  return _.flatMap(config.documentExported ? [walkExported] : [walkComments.bind(null, 'leadingComments', true), walkComments.bind(null, 'innerComments', false), walkComments.bind(null, 'trailingComments', false)], function (fn) {
    return fn(ast, data, addComment);
  }).filter(function (comment) {
    return comment && !comment.lends;
  });
}

function _addComment(visited, commentsByNode, data, commentValue, commentLoc, path, nodeLoc, includeContext) {
  ...
  if (!visited.has(key)) {
    ...
    var comment = parse(commentValue, commentLoc, context);
    if (includeContext) {
      ...
      if (t.isClassMethod(path) && path.node.kind === 'constructor') {
        ...
        if (comment.tags.some(function (tag) {
          return tag.title !== 'param' && tag.title !== 'hideconstructor';
        })) {
          debuglog('A constructor was documented explicitly: document along with the class instead');
        }

        var parentComment = commentsByNode.get(path.parentPath.parentPath.node);
        if (parentComment) {
