 ...
 */
var isEvent = function isEvent(member) {
  ...
  comments.forEach(function (comment) {
    ...
    if (!path) {
      ...
      if (comment.memberof) {
        ...
        path = comment.memberof.split('.').map(function (segment) {
          return { scope: 'static', name: segment };
        });
      }
    ...
    while (path.length) {
      ...
      if (!hasOwnProperty.call(node.members[scope], name)) {
        ...
        if (segment.toc && node !== root && hasOwnProperty.call(root.members[scope], name)) {
          node.members[scope][name] = root.members[scope][name];
          namesToUnroot.push(name);
        } else {
          var newNode = node.members[scope][name] = {
            comments: [],
            members: getMembers()
          };
  ...
  function toComments(nodes, root, hasUndefinedParent, path) {
    ...
    for (var name in nodes) {
      ...
      for (scope in node.members) {
        node.members[scope] = toComments(node.members[scope], root || result, !node.comments.length, node.comments.length && node.comments[0].kind !== 'note' ? path.concat(node.comments[0]) : []);
      }
