...
function inferModuleName(comment) {
  return comment.kind === 'module' && comment.name || pathParse(comment.context.file).name;
}
    ...
    ThisExpression(path) {
      ...
      if (n.isFunctionDeclaration(scope.block)) {
        ...
        if (scope.path.parentPath.isVariableDeclarator()) {
          ...
          identifiers = identifiers.concat(scope.path.parentPath.get('id').node.name).concat('prototype');
        } else if (scope.path.parentPath.isAssignmentExpression()) {
          /** this.Bar = function(foo) { this.foo = foo; }; */
          identifiers = identifiers.concat(extractIdentifiers(scope.path.parentPath.get('left'))).concat('prototype');
        }
      }
    }
  });
...
module.exports = function () {
  ...
  return function inferMembership(comment) {
    ...
    if (path.isExpressionStatement() && path.get('expression').isAssignmentExpression() && path.get('expression').get('left').isMemberExpression()) {
    ...
    if (path.isMemberExpression()) {
      var memberIdentifiers = [].concat(extractThis(path, comment), extractIdentifiers(path));
    ...
    if ((path.isClassMethod() || path.isClassProperty()) && path.parentPath.isClassBody() && path.parentPath.parentPath.isClass()) {
      var scope = 'instance';
      ...
      if (path.parentPath.parentPath.isExpression()) {
        return inferMembershipFromIdentifiers(comment, extractIdentifiers(path.parentPath.parentPath.parentPath.get('left')), scope);
      }
    ...
    var objectParent = void 0;

    if (path.isIdentifier() && path.parentPath.isObjectProperty() && path.parentPath.parentPath.isObjectExpression()) {
    ...
    if (objectParent) {
      ...
      var lendsIdentifiers = findLendsIdentifiers(objectParent) || findLendsIdentifiers(objectParent.get('properties')[0]);
      ...
      if (lendsIdentifiers) {
        ...
        // Foo.prototype = { ... };
        // Foo.bar = { ... };
        return inferMembershipFromIdentifiers(comment, extractIdentifiers(objectParent.parentPath.get('left')));
      } else if (objectParent.parentPath.isVariableDeclarator()) {
        // var Foo = { ... };
        return inferMembershipFromIdentifiers(comment, [objectParent.parentPath.get('id').node.name]);
      } else if (objectParent.parentPath.isExportDefaultDeclaration()) {
        // export default { ... };
        return inferMembershipFromIdentifiers(comment, [inferModuleName(currentModule || comment)]);
      }
    }
