...
module.exports.builder = Object.assign({}, sharedOptions.sharedOutputOptions, sharedOptions.sharedInputOptions, {
  output: {
    describe: 'output location. omit for stdout, otherwise is a filename ' + 'for single-file outputs and a directory name for multi-file outputs like html',
    default: 'stdout',
    alias: 'o'
  }
});
...
module.exports.handler = function build(argv) {
  ...
  function generator() {
    return documentation.build(argv.input, argv).then(function (comments) {
      return documentation.formats[argv.format](comments, argv).then(onFormatted);
    }).catch(function (err) {
      /* eslint no-console: 0 */
  ...
  function updateWatcher() {
    ...
    documentation.expandInputs(argv.input, argv).then(function (files) {
      return watcher.add(files.map(function (data) {
