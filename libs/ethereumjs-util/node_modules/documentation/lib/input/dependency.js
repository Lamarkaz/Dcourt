...
function dependencyStream(indexes, config) {
  var md = mdeps({
    ...
    filter: function filter(id) {
      return !!config.external || moduleFilters.internalOnly(id);
    },
    extensions: [].concat(config.requireExtension || []).map(function (ext) {
      return '.' + ext.replace(/^\./, '');
    }).concat(['.mjs', '.js', '.json', '.es6', '.jsx']),
    transform: [babelify.configure({
      sourceMap: false,
      compact: false,
      presets: [require('babel-preset-env'), require('babel-preset-stage-0'), require('babel-preset-react')],
  ...
  smartGlob(indexes, config.parseExtension).forEach(function (index) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    md.pipe(concat(function (inputs) {
      resolve(inputs.filter(function (input) {
        return (
          ...
          // files away.
          config.parseExtension.indexOf(path.extname(input.file).replace(/^\./, '')) > -1
        );
      }).map(function (input) {
