...
var _parse_to_ast = require('../parsers/parse_to_ast');
    ...
    ExportDeclaration(path) {
      ...
      if (t.isExportNamedDeclaration(path)) {
        ...
        specifiers.forEach(function (specifier) {
          ...
          var exported = specifier.node.exported.name;
...
function getCachedData(dataCache, filePath) {
  ...
  if (!value) {
    ...
    var ast = (0, _parse_to_ast.parseToAst)(input);
    value = {
      data: {
        file: path,
        source: input
    ...
    ExportDeclaration(path) {
      if (name === 'default' && path.isExportDefaultDeclaration()) {
        ...
        if (t.isDeclaration(declaration)) {
          ...
          if (declaration.isFunctionDeclaration() || declaration.isClassDeclaration() || declaration.isTypeAlias()) {
        ...
        for (var i = 0; i < specifiers.length; i++) {
          ...
          if (exported === name) {
            if (source) {
              ...
              var _tmp = findExportDeclaration(dataCache, local, exportKind, depPath, source.value);
              rv = _tmp.ast;
              data = _tmp.data;
              if (!rv) {
