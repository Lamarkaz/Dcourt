...
var client = artifacts.require('./testClient.sol')
var eth = require('../libs/ethereumjs-util');
const blockMiner = require('utility');
...
it("can not register with duration less than minimum global trial period", function(){
  return DCT.new().then(async function(contract){
    ...
    await DCA.register("e3b98a4da31a127d4bde6e43033f66ba274cab0eb7eb1c70ec41402bf6273dd8", 30, "http://twew.com", {from: accounts[1]}).catch(()=>{console.log("Yuppity")});
  })
})
...
it("can not vote before voting period", function(){
  return DCT.new().then(async function(contract){
    var DCA = await DCArbitration.new(contract.address, 5,5,50,1,50);
    var client = await client.new(DCA.address);
    await contract.unpause({from: accounts[0]});
    await contract.mint(accounts[1], 50, {from: accounts[0]});
    await contract.transferOwnership(DCA.address, {from: accounts[0]});
    await DCA.deposit({from: accounts[1]});
    await client.createVideo("test", {from: accounts[4]});
    await client.claimVideo(1, {from: accounts[3]});
    var hash = await DCA.generateHash("nonce", true, {fromo: accounts[1]});
    await DCA.vote(1, hash, 25, {fromo: accounts[1]});
    assert.equal((await getVoteWeight(1)).toNumber(),0);
  })
})
